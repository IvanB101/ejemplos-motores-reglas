...
public class Rulebook {
    ...
    public Rulebook(Servicio servicio) {
        ruleBook = RuleBookBuilder.create().withResultType(Double.class).withDefaultResult(0.0)
                .addRule(rule -> rule.withFactType(Afiliacion.class)
                        .when(facts -> facts.getOne().getCategoria() == Categoria.FAMILIAR
                                && facts.getOne().getSubcategoria() == com.example.base.Afiliacion.Subcategoria.CONYUGE)
                        .then((facts, result) -> {
                            result.setValue(calcularCuota(servicio.getConyuge(facts.getOne().getId())) * 0.7);
                        }).stop())
                .addRule(rule -> rule.withFactType(Afiliacion.class)
                        .when(facts -> facts.getOne().getCategoria() == Categoria.FAMILIAR
                                && facts.getOne().getSubcategoria() == Subcategoria.DESCENDIENTE_PRIMER_GRADO)
                        .then((facts, result) -> {
                            result.setValue(0.0);
                        }).stop())
                .addRule(rule -> rule.withFactType(Afiliacion.class)
                        .when(facts -> facts.getOne().getCategoria() == Categoria.VOLUNTARIO_ADHERENTE
                                && facts.getOne().getSubcategoria() == Subcategoria.BECARIO)
                        .then((facts, result) -> {
                ...
                .addRule(rule -> rule.withFactType(Afiliacion.class)
                        .when(facts -> facts.getOne().getCategoria() == Categoria.VOLUNTARIO_ADHERENTE
                                && facts.getOne().getSubcategoria() == Subcategoria.AGENTE_UNSL_CON_LICENCIA)
                        .then((facts, result) -> {
                            Afiliacion afiliado = facts.getOne();
                            if (afiliado.getEdad() <= 30) {
                                result.setValue(servicio.getHaberPercibido(afiliado.getId()) * 0.4);
                            } else if (afiliado.getEdad() <= 45) {
                                result.setValue(servicio.getHaberPercibido(afiliado.getId()) * 0.78);
                            } else if (afiliado.getEdad() <= 55) {
