package com.example.easy_rules;
...
public class EasyRules {
    ...
    public EasyRules(Servicio servicio) {
        ...
        reglas.register(new RuleBuilder()
                .name("conyuge")
                .when(facts -> ((Afiliacion) facts.get("afiliado")).getCategoria() == Categoria.FAMILIAR
                        && ((Afiliacion) facts.get("afiliado")).getSubcategoria() == Subcategoria.CONYUGE)
                .then(facts -> facts.put("monto",
                        calcularCuota(servicio.getConyuge(((Afiliacion) facts.get("afiliado")).getId())) * 0.7))
                .build());
        reglas.register(new RuleBuilder()
                .name("descendiente de primer grado")
                .when(facts -> ((Afiliacion) facts.get("afiliado")).getCategoria() == Categoria.FAMILIAR
                        && ((Afiliacion) facts.get("afiliado"))
                                .getSubcategoria() == Subcategoria.DESCENDIENTE_PRIMER_GRADO)
                .then(facts -> facts.put("monto", 0.0))
                .build());
        reglas.register(new RuleBuilder()
                .name("becario")
                .when(facts -> ((Afiliacion) facts.get("afiliado")).getCategoria() == Categoria.VOLUNTARIO_ADHERENTE
                        && ((Afiliacion) facts.get("afiliado")).getSubcategoria() == Subcategoria.BECARIO)
                .then(facts -> {
        ...
        reglas.register(new RuleBuilder()
                ...
                .then(facts -> {
                    ...
                    if (afiliado.getEdad() <= 30) {
                        monto = servicio.getHaberPercibido(afiliado.getId()) * 0.4;
                    } else if (afiliado.getEdad() <= 45) {
                        monto = servicio.getHaberPercibido(afiliado.getId()) * 0.78;
                    } else if (afiliado.getEdad() <= 55) {
                        monto = servicio.getHaberPercibido(afiliado.getId()) * 0.95;
                    } else if (afiliado.getEdad() <= 65) {
                        monto = servicio.getHaberPercibido(afiliado.getId()) * 1.13;
                    } else {
                        monto = servicio.getHaberPercibido(afiliado.getId()) * 1.86;
                    }
                    facts.put("monto", monto);
                })
                .build());
